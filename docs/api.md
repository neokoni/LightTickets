...
- `formData`：严格按所选模板声明的字段提交；未知字段、缺失必填字段、非法选项和超长值会被拒绝。
  dropdown 使用 `显示文本|实际值` 时提交实际值；滚动升级期间也接受旧客户端提交的整条选项并归一化为实际值
...
跳过、重建或重新执行决策；待决策记录会保持可提交，已完成记录会继续保留。
...
配置顺序下发多个动作。dropdown 使用 `显示文本|实际值` 时提交实际值；旧客户端提交的整条选项
也会被归一化为实际值。已完成的记录再次提交返回 `409`，不会重复下发动作。

### 跳过完成钩子决策

`POST /api/tickets/:id/completion-hooks/:hookId/skip`

需要 `staff` 权限。该操作表示本次选择“不做决策”：仅将待处理记录原子地标记为
`skipped`，不会提交字段、创建或执行 Minecraft 动作，但会记录一条“选择不做决策”的审计日志。
已完成或已跳过的记录再次操作返回 `409`。
...
实际操作人为 actor 追加“完成了决策”，选择跳过时追加“选择不做决策”。
...
可选认证，返回议题审计日志列表。`status_change` 事件会立即写入；其他事件会延迟最多约 15 秒，
并按目标与动作在窗口内合并为一条净变更。窗口内被撤销的变更不会落库，因此刚提交的非状态变更
可能暂时不可见。
