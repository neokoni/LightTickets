...
<template>
  <header
    class="sticky top-0 z-50 border-b border-slate-200/80 dark:border-slate-800/80 bg-white/45 dark:bg-slate-950/45 backdrop-blur-xl"
  >
    <div
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        >
          <Transition name="theme-icon" mode="out-in">
            <Icon
              :key="ui.theme"
              :icon="ui.theme === 'dark' ? 'lucide:sun' : 'lucide:moon'"
              class="h-4 w-4"
            />
          </Transition>
        </button>
        ...
        <template v-if="auth.isAuthenticated">
          <div
            ref="profileMenuRef"
            class="relative inline-flex items-center justify-center rounded-full border border-slate-200 dark:border-slate-800"
          >
            <button class="hover:opacity-80" @click="profileMenuOpen = !profileMenuOpen">
  ...
  <Transition name="fade">
    <div
      ...
      class="fixed inset-0 z-[60] flex items-start justify-center bg-slate-950/40 backdrop-blur-sm p-4 pt-[4.5rem] lg:hidden"
      @click.self="ui.mobileMenuOpen = false"
    >
      <Transition name="mobile-menu" appear>
        <div
          class="w-full max-w-xl rounded-xl border border-slate-200 bg-white/95 p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/95"
          @click.stop
        >
