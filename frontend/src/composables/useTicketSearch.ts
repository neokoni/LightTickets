...
const FILTER_DEFS: FilterDefinition[] = [
  {
    ...
    aliases: ['status', '状态'],
    label: '状态',
    color: { bg: 'bg-green-100 dark:bg-green-900/30', text: 'text-green-800 dark:text-green-300', border: 'border-green-300 dark:border-green-700' },
    collect: true,
    parse: (value, _ctx) => {
  ...
  },
  {
    ...
    aliases: ['type', '类型', '模板'],
    label: '类型',
    color: { bg: 'bg-blue-100 dark:bg-blue-900/30', text: 'text-blue-800 dark:text-blue-300', border: 'border-blue-300 dark:border-blue-700' },
    parse: (value, ctx) => {
      const v = value.toLowerCase()
      const matched = ctx.templates.find(t => t.name.toLowerCase() === v || t.name_i18n.toLowerCase() === v)
    ...
    suggestions: (ctx) => ctx.templates.map(t => ({ value: t.name, label: `${t.name} (${t.name_i18n})` })),
  },
  {
    key: 'label',
    aliases: ['label', '标签'],
    label: '标签',
    color: { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-800 dark:text-purple-300', border: 'border-purple-300 dark:border-purple-700' },
    parse: (value, ctx) => {
      const v = value.toLowerCase()
  ...
  },
  {
    ...
    aliases: ['from', '来源'],
    label: '来源',
    color: { bg: 'bg-orange-100 dark:bg-orange-900/30', text: 'text-orange-800 dark:text-orange-300', border: 'border-orange-300 dark:border-orange-700' },
    parse: (value, ctx) => {
      const v = value.toLowerCase()
    ...
    suggestions: (ctx) => [
      ...
      ...ctx.servers.map(s => ({ value: `minecraft:${s.name}`, label: `minecraft:${s.name} (${s.name}服务器)` })),
    ],
  },
  {
    key: 'author',
    aliases: ['author', '作者'],
    label: '作者',
    color: { bg: 'bg-cyan-100 dark:bg-cyan-900/30', text: 'text-cyan-800 dark:text-cyan-300', border: 'border-cyan-300 dark:border-cyan-700' },
...
export function statusToFilterToken(status: TicketStatus): string {
