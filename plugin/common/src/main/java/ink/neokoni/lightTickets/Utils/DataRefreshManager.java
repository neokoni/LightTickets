...
public class DataRefreshManager {
    ...
    public static void start() {
        ...
        LightPlatformProvider.get().scheduleAtFixedRate(DataRefreshManager::processOne,
                3, 3, TimeUnit.SECONDS);

        LightPlatformProvider.get().getLogger().info(LangUtils.getRawLang("data_refresh.started",
    ...
    public static void refreshNow(UUID uuid) {
        if (uuid == null || LightPlatformProvider.get().getPlayer(uuid) == null) return;
        RefreshState state = stateFor(uuid);
        synchronized (state) {
    ...
    private static void processOne() {
        ...
        synchronized (state) {
            state.queued = false;
            if (!state.active || LightPlatformProvider.get().getPlayer(uuid) == null) {
                state.active = false;
                if (!state.inFlight) {
                    refreshStates.remove(uuid, state);
                }
    ...
    private static boolean hasUsableCredential(LightPlayer player) {
